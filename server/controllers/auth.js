...
const resetPasswordSchema = Joi.object({
  email: Joi.string().required().email().messages(constants.ERROR_MESSAGE_EMAIL),
  password: Joi.string().required().min(8).messages(constants.ERROR_MESSAGE_PASSWORD)
});
...
module.exports.resetPassword = (req, res, next) => {
  if (!config.auth.resetPassword) {
    return next(createError(422, 'Password reset functionality is not available'));
  }
...
const sendTokenSchema = Joi.object({
  ...
  tokenPurpose: Joi.string()
    .required()
    .valid(constants.TOKEN_PURPOSE_VERIFY_EMAIL, constants.TOKEN_PURPOSE_RESET_PASSWORD)
});
    ...
    .then((payload) => {
      ...
      if (req.body.tokenPurpose === constants.TOKEN_PURPOSE_VERIFY_EMAIL) {
        if (!config.auth.verifyEmail) {
          return next(createError(422, 'Email verification functionality is not available'));
        }
...
module.exports.invalidateAllJwtTokens = (req, res, next) => {
  if (req.user) {
    ...
    return req.user
      .save()
      .then((user) => {
        res.status(200).json({ message: 'All JWT tokens have been invalidated' });
      })
      .catch(next);
  }
};
 ...
 * Validate local sign-in payload. Either email or username must be specified.
 *
...
/**
 * JOI schema for validating Google signIn payload
 */
const googleSignInSchema = Joi.object({
  ...
  data: Joi.object({
    //TODO: Still need to unpack some information from the Google response
    username: Joi.string()
      // .required()
      .pattern(/^[a-zA-Z0-9.\-_]{4,30}$/)
      .messages(constants.ERROR_MESSAGE_USERNAME),
    ...
    userId: Joi.string(),
    password: Joi.string().min(8).messages(constants.ERROR_MESSAGE_PASSWORD)
 ...
 *
 * @param {string} req.headers.authorization The Google access token
 * @param {string} req.data.username The username to login
 * @param {string} req.data.email The email to login
 * @param {string} req.data.userId The Google user id
 */
module.exports.signInWithGoogle = async (req, res, next) => {
  googleSignInSchema.validateAsync(req.body).then((payload) => {
    req.body = payload;
    const { email, username, userId } = req.body.data;
    ...
    return (
      User.findOne({
        ...
        // If user exists, update it with the userId
        .then((user) => {
          if (user) {
            user.userId = userId;
            return user.save();
          ...
          return User.create({
            ...
            username,
            userId,
            signedInWithProvider: constants.SIGNED_IN_WITH_GOOGLE
          });
        })
        // Return user object
        .then((user) => {
          req.user = user;
          if (req.user) {
            res.status(200).json(req.user.toJsonFor(req.user));
          }
          return user;
        })
        .catch(next) // Catch any error
    );
  });
};
...
const sendPasswordResetToken = (req, res, next) => {
  User.findOne({ email: req.body.email })
    .then((user) => {
      if (!user) {
        throw createError(422, 'Email not associated with any account');
      }
 ...
 * @returns {Promise} Resolve with a sending result object
 */
const sendEmailHelperAsync = (user, subject, title, content, buttonText, url) => {
  return email.send({
    to: user.email,
