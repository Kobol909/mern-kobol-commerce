 ...
 * @param {string} [req.query.phone] The phone
 * @param {string} [req.query.address] The address
 * @param {string} [req.query.address2] The address2
 * @param {string} [req.query.city] The city
 * @param {string} [req.query.state] The state
 * @param {string} [req.query.zip] The zip
 * @param {string} [req.query.country] The country
...
module.exports.getUsers = (req, res, next) => {
  getUsersSchema
    ...
    .then((payload) => {
      ...
      return Promise.all([
        User.find(query).sort(req.query.sort).limit(req.query.limit).skip(req.query.skip).exec(),
...
const updateUserSchema = Joi.object({
  role: Joi.string().valid(constants.ROLE_ADMIN, constants.ROLE_ROOT, constants.ROLE_USER),
  status: Joi.string().valid(
    constants.STATUS_ACTIVE,
    constants.STATUS_DISABLED,
    constants.STATUS_UNVERIFIED_EMAIL
...
module.exports.updateUser = (req, res, next) => {
  updateUserSchema
    ...
    .then(() => {
      if (req.body.role) {
        if (
          (req.user.role === constants.ROLE_ADMIN && req.body.role !== constants.ROLE_USER) ||
          (req.user.role === constants.ROLE_USER && req.body.role !== constants.ROLE_USER)
        ) {
      ...
      return res.locals.targetUser.update(), res.locals.targetUser.save();
    })
    .then((updatedUser) => {
      res.status(200).json({
